...
function FedNowValidationRulesPain013(Pain013ValdFlagMx, exchange){
	...
	/*	try {
        ...
        if (retVal != 0) {
			...
			retVal = fedNowPreferredContactMethodRule1Pain013(exchange);
        if (retVal != 0) {
            return retVal;
        }
...
function fedNowRequestedExecutionDateTimeRule1Pain013(exchange) {
    ...
    var retVal = 0;
    var msgDate;
    logger.info(" fedNowRequestedExecutionDateTimeRule1Pain013");
    var inMsg = exchange.getIn();
    var map = inMsg.getHeaders();
    var Document = exchange.getIn().getBody(org.w3c.dom.Document.class);
...
function fedNowAccountTypeGuidelinePain013(exchange) {
	...
	if(cRetVal) {
		...
		setHeader(map, "PLCN_validMessage", false);
		cRetVal = setCommentsForTransaction("175", "1556", map);
		cRetVal = 1;
    }
    ...
    if(dRetVal) {
		...
		setHeader(map, "PLCN_validMessage", false);
		dRetVal = setCommentsForTransaction("628", "1556", map);
		dRetVal = 1;
    }
...
function fedNowRemittanceInformationRule2Pain013(exchange) {
   ...
   unStructuredPathValue= getValueFromPath(Document,  unStructuredPath);
	...
	if ((structuredPathValue && unStructuredPathValue) || (!structuredPathValue && !unStructuredPathValue)) {
		validflag = false;
		logger.info("Both Structured and unstructured values are there");
		setHeader(map, "PLCN_validMessage", false);
		retVal = setCommentsForTransaction("148", "7987", map);
...
function fedNowLocalInstrumentRule1Pain013(exchange) {
	logger.info("LocalInstrumentRule1Pain013");
	var lclInstrm;
	var lclInstrmPath;
	var retVal = 0;
    var validflag;
...
function fedNowCategoryPurposeRulePain013 (exchange) {
	...
	if( value )
	{
		...
		if(uValue == "CONS" || uValue == "BIZZ" || uValue == "GOVT" ) 
		{
			...
			setHeader(map, "PLCN_validMessage", false);
			retVal = setCommentsForTransaction("322", "7984", map);
			return retVal;
		}
	} else {
		setHeader(map, "PLCN_validMessage", false);
		logger.info( "CategoryPurposeRulePain013 is failure");
			retVal = setCommentsForTransaction("210", "7984", map);
			return retVal;
	}
...
function fedNowPreferredContactMethodRule1Pain013(exchange){

    logger.info("In PreferredContactMethodRule1Pain013 ")
    var inMsg = exchange.getIn();
    var map = inMsg.getHeaders();
    var retVal = 0;
    var validFlag;
    ...
    if (prefInvcrContactMethodValue) {
        ...
        if (prefInvcrContactMethodValue == "MAIL") {
            path = "/Document/CdtrPmtActvtnReq/PmtInf/CdtTrfTx/RmtInf/Strd/Invcr/CtctDtls/EmailAdr";
            var value = getValueFromPath(Document, path);
            logger.info("PreferredContactMethodRule1Pain013 : EmailId " + value);
            if (value) {
                logger.info("PreferredContactMethodRule1Pain013 EmailId passed " + value);
                validFlag = true;
            }
            else {
                logger.info("PreferredContactMethodRule1Pain013 EmailId failed " + value);
                setHeader(map, "PLCN_validMessage", false);
                retVal = setCommentsForTransaction("366", "7986", map);
                retVal = 1;
            }
        }
    ...
    {
        logger.info("PreferredContactMethodRule1Pain013 passed :" +value);
        validFlag = true;
    }
...
function fedNowPreferredContactMethodRule2Pain013(exchange){
    ...
    if (prefInvcrContactMethodValue) {
        ...
        if (prefInvcrContactMethodValue == "CELL") {
            path = "/Document/CdtrPmtActvtnReq/PmtInf/CdtTrfTx/RmtInf/Strd/Invcr/CtctDtls/MobNb";
            var value = getValueFromPath(Document, path);


            logger.info("PreferredContactMethodRule1Pain013 : CELL " + value);
            if (value) {
                logger.info("PreferredContactMethodRule1Pain013 CELL passed " + value);
                validFlag = true;
            }
            else {
                logger.info("PreferredContactMethodRule1Pain013 CELL failed " + value);
                setHeader(map, "PLCN_validMessage", false);
...
function fedNowPreferredContactMethodRule3Pain013(exchange){
    ...
    if (prefInvcrContactMethodValue) {
        ...
        if (prefInvcrContactMethodValue == "PHON") {
            path = "/Document/CdtrPmtActvtnReq/PmtInf/CdtTrfTx/RmtInf/Strd/Invcr/CtctDtls/PhneNb";
            var value = getValueFromPath(Document, path);
    ...
    {
        logger.info("PreferredContactMethodRule1Pain013 passed :" +pathValue);
        validFlag = true;
    }
