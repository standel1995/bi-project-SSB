...
function grpHdr_CdtTrfTxInf_FldCompRulePacs008(Document, map) {
    ...
    logger.info("grpHdr_CdtTrfTxInf_FldCompRulePacs008: msgFamily = " + msgFamily);
...
function genericNotAllowedRulePacs004(Document, map) {
	...
	var txInfInstdAgt = isXmlNodePresent(Document, "PmtRtr", "TxInf", "<InstdAgt>");
	...
	var txInfInstgAgt = isXmlNodePresent(Document, "PmtRtr", "TxInf", "<InstgAgt>");
...
function chargesInfoSepaPacs007Rule(Document,map){
	...
	if(rvsdInstdAmt && !chrgsInf){
		setHeader(map, "PLCN_validMessage",false);
		logger.info("chargesInfoSepaPacs007Rule: RvsdInstdAmt Only allowed in the case of Amt under ChrgsInf is included.");
		retVal = setCommentsForTransaction("049", "8039", map);
		return retVal;
	}
	return retVal;
}
